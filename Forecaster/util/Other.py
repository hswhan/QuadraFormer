...
class FourierLayer(nn.Module):
    ...
    def topk_freq(self, x_freq):
        if x_freq.numel() == 0:
            raise ValueError("x_freq is empty")

        if torch.isnan(x_freq).any() or torch.isinf(x_freq).any():
            raise ValueError("x_freq contains NaN or Inf")

        # 假设 x_freq.shape = (B, N, D)
        B, N, D = x_freq.shape
        values, indices = torch.topk(x_freq.abs(), self.k, dim=1, largest=True, sorted=True)  # indices: (B, k, D)

        # 构建 meshgrid 索引
        batch_idx = torch.arange(B).view(B, 1, 1).expand(B, self.k, D)
        feat_idx = torch.arange(D).view(1, 1, D).expand(B, self.k, D)
        index_tuple = (
        batch_idx.to(x_freq.device), indices.to(x_freq.device), feat_idx.to(x_freq.device))  # aligned shape

        # 选择频率最大的值
        x_freq = x_freq[index_tuple]  # shape: (B, k, D)
    ...
    def dft_forward(self, x):
        ...
        i, j = torch.meshgrid(torch.arange(self.pred_len + T), torch.arange(T), indexing = 'ij')
