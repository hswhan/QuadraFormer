...
class MEB(nn.Module):
    def __init__(self, input_size, output_size, num_experts, device, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
        ...
        for patch in patch_size:
            ...
            self.experts.append(QuadraLayer(device=device, d_model=d_model, d_ff=d_ff,
    ...
    def _prob_in_top_k(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        normal = torch.distributions.Normal(torch.tensor([0.0], device=clean_values.device),
    ...
    def forward(self, x, loss_coef=1e-4):
        ...
        # 使用 torch.jit.fork 并行执行 Transformer 层
        futures = [torch.jit.fork(lambda x, i=i: self.experts[i](x)[0], expert_inputs[i]) for i in
                   range(self.num_experts)]
        # 等待结果合并
        expert_outputs = [torch.jit.wait(f) for f in futures]
